...
./request_tcp.sh [주기ms=1] [임계배수=3.0]
```
...
```
첫 이벤트 수신, seq=1748
[loss] seq 1749 -> 1750: 복구 시간 2745us (수신 간격 3745us)
...
총 수신 2744건, 유실/지연 감지 81건
```

**주기 기본값(1ms)이 왜 이렇게 짧은지, 임계배수(3.0배)가 왜 이렇게 큰지는 아래 "발행 주기가 왜
중요한가" 참고** - 요약하면 vsomeip의 TCP 발행 자체가 요청 주기와 무관하게 실제로는 그 ~6.7배
간격으로만 나간다(이 환경 실측치), 그래서 임계배수도 "주기"가 아니라 "실제 간격" 기준으로 잡아야
오탐이 안 생긴다.

**주기를 바꿨다면 먼저 기준선을 다시 확인할 것.** `NO_DROP=1 sudo ./response_tcp.sh <클라이언트 IP>`로
유실 주입 없이 돌려서 이 링크·이 주기에서 평소 수신 간격이 얼마나 벌어지는지 본 다음, `--threshold`가
그보다 확실히 높은지 확인한다. 기본값(`--cycle 1 --threshold 3.0`)은 이 프로젝트의 유선 직결 링크에서
오탐 0/2828건으로 확인됐다.
...
|---|---|---|---|---|---|---|
| 방법 2 (와이어 레벨), 튜닝 없음 | 5ms | 45 | 2.70ms | **0.57ms** | 5.63ms | 58% |
| 방법 2 (와이어 레벨), + `tune_tcp_recovery.sh` | 5ms | 47 | 2.39ms | **0.57ms** | 5.63ms | 64% |
| 방법 1 (애플리케이션 레벨), 구 기본값 | 5ms, 임계 2.0 | 12 | 7.43ms | - | - | - |
| 방법 1 (애플리케이션 레벨), + 튜닝 | 5ms, 임계 2.0 | 11 | 7.78ms | - | - | - |
| **방법 1 (애플리케이션 레벨), 현재 기본값** | **1ms, 임계 3.0** | 71 / 81 (재현 2회) | **2.73ms / 2.76ms** | 2.75ms | - | - |

와이어 레벨 중앙값이 아래 Wi-Fi 시절 수치(1.64ms)보다 크게 낮아졌다(0.57ms) - 대부분의 재전송은 이제
2ms를 훌쩍 넘기지 않는다. 애플리케이션 레벨(방법 1)은 `--cycle 5`(구 기본값)로는 링크를 바꿔도 커널을
튜닝해도 거의 그대로였다(7.4~7.8ms) - 원인과 고친 값(`--cycle 1 --threshold 3.0` → 2.7ms, 방법 2와
거의 일치)은 아래 "발행 주기가 왜 중요한가" 참고. 두 방법 다 오탐 0건(`NO_DROP=1`으로 확인).

남은 꼬리(방법 2의 p90 5.6ms)는 튜닝해도 거의 안 줄었다 - 아래 "2ms까지 남은 구간"에서 설명하는 커널 틱
