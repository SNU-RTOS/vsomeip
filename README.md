...
| TCP 패킷 복구 시간       | 평균 10ms    | 평균 4ms      | 평균 3.46ms(와이어) / 9.7ms(체감) | **중앙값 0.57ms**(와이어) / 7.4~7.8ms(체감, 거의 안 바뀜) |

3차는 Thor↔Orin을 스위치 대신 이더넷 케이블로 직결한 뒤의 수치 - 자세한 내용, 왜 두 지표(와이어/체감)가
따로 있는지, 왜 체감 값은 거의 안 바뀌었는지는 [CHANGES_THOR.md](CHANGES_THOR.md) "Direct Ethernet
Link"와 [tcp-recovery/README.md](tcp-recovery/README.md) "결과: 실측값" 참고. 2차(Wi-Fi) 수치는 비교
기준으로 남겨뒀다. 1차 수치("평균 4ms")는 측정 방법의 결함(`OUTPUT` DROP이 실제로는 선로 유실을
흉내내지 못하는 문제 등) 때문에 이후 수치와 직접 비교하기는 어렵다.
